...
from .encrypt import decrypt_aes_cbc, get_key_and_iv, encrypt_symmetric_key, to_base64, encrypt_invoice, calculate_hash, encrypt_padding
...
class KSEFSDK(HOOKHTTP):
    INVOICES = namedtuple(
        typename="invoices_ksef", field_names=["ok", "ordinalNumer", "msg", "invoiceNumber", "ksefNumber", "referenceNumber"]
    )
        ...
        def decorator(func):
            def wrapper():
                ...
                for attempt in range(max_retries):
                    ...
                    sleep(sleep_time)
                    sleep_time += 2
                raise TimeoutError(f"Przekroczono maksymalną liczbę prób: {max_retries}")
    ...
    def _verify_environment(env: int):
        if env not in KSEFSDK._env_dict:
            raise ValueError("Niepoprawne środowisko, parametr env musi mieć wartość DEVKSEF, PREKSEF lub PRODKSEF")
    ...
    def __init__(self, url: str, nip: str, A: ABSTRACTTOKEN, unittest: bool):
        ...
        A.set_params(timestamp=timestamp, kseftoken_certificate=kseftoken_certificate)
        self._referencenumber, self._authenticationtoken = A.auth_ksef(H=self, nip=nip, challenge=challenge)
    ...
    def _get_public_certificate(self) -> tuple[str, str]:
        response = self.hook("security/public-key-certificates", method=self._METHODGET, bearer=self._NOBEARER)
        kseftoken_certificate = next(e["certificate"] for e in response if "KsefTokenEncryption" in e["usage"])
        symmetrickey_certificate = next(e["certificate"] for e in response if "SymmetricKeyEncryption" in e["usage"])
    ...
    def _session_status(self) -> None:
        ...
        def _status():
            response = self.hook(url, method=self._METHODGET, bearer=self._BEARERTOKEN)
    ...
    def _redeem_token(self) -> tuple[str, str]:
        response = self.hook(endpoint="auth/token/redeem", bearer=self._BEARERTOKEN)
    ...
    def _prepare_session_data(self) -> dict:
        encrypted_symmetric_key = encrypt_symmetric_key(
            symmetricy_key=self._symmetric_key, public_certificate=self._symmetrickey_certificate
        )
        request_data = {
            "formCode": {"systemCode": "FA (3)", "schemaVersion": "1-0E", "value": "FA"},
            "encryption": {"encryptedSymmetricKey": to_base64(encrypted_symmetric_key), "initializationVector": to_base64(self._iv)},
        }
        return request_data

    def _prepare_query(self, subject: str, date_from: str, date_to: str) -> dict:
        query = {"subjectType": subject, "dateRange": {"dateType": "PermanentStorage", "from": date_from, "to": date_to}}
        return query
    ...
    def _invoice_status(self, end_point: Optional[str] = None) -> tuple[bool, str, str]:
        ...
        def _status() -> tuple[bool, str, str]:
            ...
            if code == 100 or code == 150:
                ...
                return None
            if code == 200:
                self._invoicereferencenumber = self._reference_number(response) if is_interactive else ""
    ...
    def send_invoice(self, invoice: str) -> tuple[bool, str, str]:
        invoice_len, encrypted_invoice = encrypt_invoice(symmetric_key=self._symmetric_key, iv=self._iv, invoice=invoice)
    ...
    def pobierz_upo(self, invoicereferencenumber: Optional[str] = None) -> str:
        ...
        response = self.hook_response(endpoint=end_point, method=self._METHODGET)
        return response.text

    def get_list_of_tokens(self) -> list[dict]:
        end_point = "tokens"
        response = self.hook(endpoint=end_point, method=self._METHODGET)
    ...
    def send_batch_session_bytes(self, payload: Iterator[bytes], wez_upo: Optional[Callable] = None) -> tuple[bool, str, list[INVOICES]]:
        ...
        for partno, b in enumerate(payload):
            ...
            encrypted_data = encrypt_padding(symmetric_key=self._symmetric_key, iv=self._iv, b=b)
        ...
        request_data.update({"batchFile": {"fileSize": fileSize, "fileHash": to_base64(crc.digest()), "fileParts": fileParts}})
        ...
        try:
            for p in partUpload:
                ...
                _l(msg)
                self.hook_response(endpoint=url, data=encrypted_data, requestmethod=method, requestheaders=headers)
        finally:
        ...
        for i in invoices:
            ...
            if wez_upo is not None and upo is not None:
                response = self.hook_response(endpoint=upo, requestmethod="GET")
    ...
    def get_batch_invoices(self, date_from: str, date_to: str, subject: str) -> tuple[int, bytes]:
        request_data = self._prepare_session_data()
        query = self._prepare_query(subject=subject, date_from=date_from, date_to=date_to)
        ...
        def _get_exported_invoices() -> dict:
            ...
            if code == 200:
                ...
                raise ValueError(f"Błąd podczas eksportu: {code} - {description}")
        ...
        for part in parts:
            ...
            decrypted_data = decrypt_aes_cbc(key=self._symmetric_key, iv=self._iv, b=encrypted_data)
